...
bool SaveSystem::LoadFromDisk(std::string str) {
    ...
    std::string name;
    std::string job;
    int maxhp, hp, maxmp, mp, level, money, exp, def, statpts;
    std::vector<int> stats(5);
    
    try {
        f >> name >> job >> maxhp >> hp >> maxmp >> mp >> level >> money >> exp
    ...
    player loadedplayer(name, job, maxhp, maxmp, level, exp, statpts, money);
